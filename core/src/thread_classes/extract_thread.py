...
from core.src.structs_classes.painting_work_structs.extract_structs import PerInfo, PerWorkList
...
class RestoreThread(threading.Thread):
    ...
    def __init__(self, id_thread, name, able: PerWorkList, unable: PerWorkList, parent: MainFrame, setting,
                 names, save_path, work_func=ImageWork.p_restore_tool):
        ...
        for self.index in range(len(self.able)):
            ...
            if self.index < len(self.able) and not self.stop:
                ...
                if self.setting[data.sk_output_group] == data.feg_do_no_group:
                    ...
                    pattern_skin = data.fp_char_texture
                    pattern_power = data.fp_npc
                    pattern_marry = data.fp_enemy
                    pattern_young = data.fp_char_painting
                    pattern_self = data.fp_char_build_up
                    pattern_skin_char = data.fp_char_skin

                    if pattern_marry.match(now_info.name) is not None:
                        save_path = f"{self.save_path}\\NPC"

                    elif pattern_power.match(now_info.name) is not None:
                        save_path = f"{self.save_path}\\敌人小人贴图集"

                    elif pattern_self.match(now_info.name) is not None:
                        save_path = f"{self.save_path}\\干员精英化立绘"

                    elif pattern_young.match(now_info.name) is not None:
                        save_path = f"{self.save_path}\\干员初始立绘"
                    elif pattern_skin_char.match(now_info.name) is not None:
                        save_path = f"{self.save_path}\\干员皮肤"
                    elif pattern_skin.match(now_info.name) is not None:
                        save_path = f"{self.save_path}\\干员小人贴图集"
                    else:
                        save_path = f"{self.save_path}\\其他"

                elif self.setting[data.sk_output_group] == data.feg_by_name:
                    val = re.match(r'^(?:npc|char|build_char|enemy)_\d{3,4}_([^_]+)(?:.+?)?$', now_info.name)
                    if val is not None:
                        val = val.group(1)
