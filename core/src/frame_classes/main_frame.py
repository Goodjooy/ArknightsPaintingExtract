...
class MainFrame(Mf):
    ...
    def __init__(self, parent, path=os.getcwd()):
        ...
        # 查找tree索引的信息
        self.pos, self.type_is, self.name, self.index = None, None, None, None
        # 设置拖动绑定
    ...
    def __search(self, value):
        ...
        :param value: 搜索关键字
        """
        is_regex = self.setting_info[self.data.sk_regex_search]
        is_inverse = self.setting_info[self.data.sk_inverse]

        if value != '':
            try:
                if not self.filter_type:
                    indexes = SearchOrder.find(value, self.painting_work.build_search(), is_regex=is_regex,
                                               is_inverse=is_inverse)
                    self.view_work = self.painting_work.build_from_indexes(indexes)
                else:
                    indexes = SearchOrder.find(value, self.view_work.build_search(), is_regex=is_regex,
                                               is_inverse=is_inverse)
        ...
        if not self.enter_exit:
            ...
            # 寻找tex或mesh路径句柄
                ...
                if is_ok:
                    ...
                    self.pos, self.type_is, self.name, self.index = pos, type_is, name, index
    ...
    def choice_file(self, event):
        if self.pos == self.data.td_single:
            self.change_path(self.pos, self.type_is, self.name)
        else:
            if self.type_is == self.data.td_texture_type:
                self.name.set_tex(self.index)
            else:
                self.name.set_mesh(self.index)
    ...
    def search(self, event):
        if self.setting_info[self.data.sk_auto_search]:
            if event is None:
    ...
    def cancel_work(self, event):
        if self.setting_info[self.data.sk_regex_search]:
